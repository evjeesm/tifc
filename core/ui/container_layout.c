        ...
        const container_layout_opts_t *const opts)
{
    ...
    *layout = (container_layout_t) {
        ...
        .areas = dynarr_create(
            .element_size = sizeof(container_area_t),
        ),
   ...
   {
        container_layout_add_area(layout, &opts->areas_def[ai]);
   }
}
...
{
    assert(layout);
    assert(ui_element_area);

    S_LOG(LOGGER_DEBUG, "Panel Area: [%u, %u][%u, %u]\n",
            ui_element_area->first.x, ui_element_area->first.y,
            ui_element_area->second.x, ui_element_area->second.y);

    const disp_pos_t ui_element_size = {
        ui_element_area->second.x - ui_element_area->first.x + 1,
        ui_element_area->second.y - ui_element_area->first.y + 1,
    };

    /* subtracting panel's border */
    const ssize_t width = ui_element_size.x - layout->padding.left - layout->padding.right;
    const ssize_t height = ui_element_size.y - layout->padding.top - layout->padding.bot;
    assert(width >= 0);
    assert(height >= 0);
    ...
    calculate_spans(ui_element_area->first.x + layout->padding.left, width, layout->columns,
            0 /* columns offset */,
            layout->layout, layout->spans);

    S_LOG(LOGGER_DEBUG, "Calculate rows:\n");
    calculate_spans(ui_element_area->first.y + layout->padding.right, height, layout->rows,
...
container_area_t *container_layout_peek_area(const container_layout_t *const layout, const disp_pos_t pos)
{
    ...
    {
        container_area_t *area = dynarr_get(layout->areas, ai);
...
static void calc_areas(dynarr_t *const areas, span_t columns[], span_t rows[])
{
    ...
    {
        container_area_t *area = dynarr_get(areas, i);
...
{
    size_t *count = param;
    const container_area_t *area = element;
